...
An environment pack is declarative content plus the tooling, docs, and evidence for
...
RAES core (`raes`): it exists to make authoring and shipping RAES scenarios
easier and defines no extensions to RAES semantics
([ADR 0021](decisions/adrs/0021-adopt-raes-environment-pack-identity.md)). It ships
...
Every new pack starts under `environments/<name>/` in a catalog repo and follows
...
## SDL Validation (through RAES)

`sdl/` holds the scenario start state as one or more `<name>.sdl.yaml` documents
authored in [RAES SDL][raes]. `raes-pack-validate` parses **every** direct
`sdl/*.sdl.yaml` through RAES (`raes.parse_sdl_file`, with full semantic
validation) and fails on a pack that ships no start-state document or any
document RAES rejects. SDL is validated *through RAES* — no SDL schema is
restated here, and the local JSON-Schema subset validator never touches SDL.

`raes` is therefore a hard, exactly-pinned runtime dependency
(the exact version lives once in [`pyproject.toml`](../pyproject.toml)) rather
than an optional one: the gate is fail-closed, so an
optional coupling that skipped SDL when RAES was absent would leave every pack's
most important content unchecked. The exact pin is deliberate while RAES SDL
contracts are `stability: draft`; advancing it requires compatibility tests. This
raises the package's Python floor to match RAES (`>=3.11`). See
[ADR 0021](decisions/adrs/0021-adopt-raes-environment-pack-identity.md).
...
`raes-pack-validate` is the author-CI surface for a catalog checkout. A consumer
...
- the referenced compatibility manifest, when present, against the packaged
  schema; and
- every direct `sdl/*.sdl.yaml` document through the pinned RAES parser.
...
The exactly pinned RAES release does not expose a resolver-policy seam for
...
- `built`: it stands up somewhere, but the declared RAES participant behavior
...
- `tests/` exercises every required RAES participant behavior and objective
  against that live range.
- `docs/walkthroughs/` is the human-readable, command-by-command version of the
  same behavior.

The three must agree on the declared RAES behavior, objectives, and observable
...
It carries **zero extensions to RAES semantics**: scoring, validation-oracle,
...
`provenance.schema.yaml` by `raes-pack-validate`. It is the canonical,
...
- **`artifacts[]`** — pack-relative roots classified for distribution as `open`,
  ...
  `customer-specific`. This open-vs-RAES-only axis is distinct from the
...
**RAES is the authority for pack trust.** An environment pack is a `reusable_scenario`
asset in the RAES reusable-asset trust policy
(`reusable-asset-trust-policy/v1`; RAES ADR-071), which owns pack integrity and
authenticity (`integrity_digest`, `authenticity_signature`,
`provenance_lock_record`, `governance_source`, `artifact_checksum`). Per
[ADR 0021](decisions/adrs/0021-adopt-raes-environment-pack-identity.md), this
ledger is scoped to pack-domain facts RAES does not define — content-origin
...
**RAES concept-authority governs concept vocabulary.** ATT&CK and ATLAS
offensive-behaviour tactics, UCO concept families, and the controlled
vocabularies are published by RAES under `contracts/concept-authority/` and are
authored and validated in RAES SDL behaviour specifications, not restated in the
...
schema to `environment-pack-provenance/v3` and the environment-pack contract to
version 3. See
[ADR 0021](decisions/adrs/0021-adopt-raes-environment-pack-identity.md).
...
to prove which concrete bytes the identity covers. RAES owns that portable model:
`associated-artifact-manifest-v1` attaches one exact payload set to a scenario
parent, `associated-artifact-set/v1` derives its set digest, and the RAES
validator binds the parent, set, checksums, sizes, and staged bytes.

A pack opting into content identity points at the RAES JSON manifest from
`pack.yaml`:
...
digest = pack_content_digest(pack_root)            # declared RAES set digest
...
parent through RAES. Neither runs git, a subprocess, pack-local code, network
...
RAES owns canonicalization and byte-binding semantics. The resulting
...
The hydrated RAES SDL is the scenario specification. Declare attacker behavior
with RAES participant semantics: bind the attacker through `agents` and
`behavior_specifications`, declare governed actions through `action_contracts`,
and use RAES preconditions, effects, failure classes, observation boundaries,
outcome interpretation rules, objectives, evidence requirements, and workflows
as the scenario requires. `raes-pack-validate` parses that SDL through the
exactly pinned `raes` package; this package does not define a second behavior
shape or decide that an RAES behavior specification is complete.

The reference build, automated rehearsal, and participant walkthrough then
demonstrate that the live environment realizes the declared RAES behavior and
...
the same RAES-backed scenario and live evidence; none is a competing
specification.
...
RAES uses **delivery/audience bundles** for operating profiles:
...
or presenter files are exposed. It does not change the hydrated RAES SDL,
